...
const indexScript = new htmlParser.HTMLElement("script", {});
indexScript.setAttributes({ type: "module", src: "./assets/" + indexJs.name + indexJs.ext });

// <link rel="stylesheet" crossorigin href="/assets/index-[hash].css">
const indexStyle = new htmlParser.HTMLElement("link", {});
...
if (input && typeof(input) === "object" && !Array.isArray(input)) {
	...
	if (outputPath) {
		...
		console.log(`Registered "${outputPath}" from "${sourcePath}" as a service worker`);
	}
}
